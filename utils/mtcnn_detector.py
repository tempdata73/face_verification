...
class MtcnnDetector(object):
    ...
    def __init__(self, model_folder='.', minsize=20,
        ...
        Parameters:
        ----------
            model_folder: (string) path for the models
            minsize: (float) minimal face to detect
            threshold (float) detect threshold for 3 stages
            factor: (float) scale factor for image pyramid
    ...
    def calibrate_box(self, bbox, reg):
        ...
        Parameters:
        ----------
            bbox: (ndarray [n x 5]) input bboxes
            reg: (ndarray [n x 4]) bboxes adjustment
    ...
    def pad(self, bboxes, w, h):
        """
        Pad the the bboxes, also restrict its size

        Parameters:
        ----------
            bboxes: (ndarray [n x 5]) input bboxes
            w: (float) width of the input image
            h: (float) height of the input image

        Returns:
        -------
            dy, dx: (ndarray [n x 1]) start point of the bbox in target image
            edy, edx : (ndarray [n x 1]) end point of the bbox in target image
            y, x : (ndarray [n x 1]) start point of the bbox in original image
            ey, ex : (ndarray [n x 1]) end point of the bbox in original image
            tmpw, tmph: (ndarray [n x 1]) height and width of the bbox
        """

        tmpw = bboxes[:, 2] - bboxes[:, 0] + 1
        tmph = bboxes[:, 3] - bboxes[:, 1] + 1
    ...
    def detect_face(self, img):
        ...
        Parameters:
        ----------
            img: (ndarray [1, 3, n, m]): bgr input image

        Returns:
        -------
            bboxes: (ndarray [n x 5]) (x1, y2, x2, y2, score)
            points: (ndarray [n x 10]) (x1, x2 ... x5, y1, y2 ..y5) landmarks
        """
        ...
        for batch in sliced_index:
            local_boxes = self.Pool.map(helper.detect_first_stage_wrapper,
                                        zip(repeat(img),
        ...
        for i in range(num_box):
            ...
            input_buf[i, :, :, :] = helper.adjust_input(
        ...
        for i in range(5):
            ...
            [dy, edy, dx, edx, y, ey, x, ex, tmpw, tmph] = self.pad(np.vstack(
                                                                    [x, y, x + patchw - 1, y + patchw - 1]).T,
                                                                    width, height)
            for j in range(num_box):
                tmpim = np.zeros((tmpw[j], tmpw[j], 3), dtype=np.float32)
                tmpim[dy[j]:edy[j] + 1, dx[j]:edx[j] + 1, :] = img[y[j]:ey[j] + 1, x[j]:ex[j] + 1, :]
                input_buf[j, i * 3:i * 3 + 3, :, :] = helper.adjust_input(
    ...
    def list2colmatrix(self, pts_list):
        ...
        assert len(pts_list) > 0, "point list must be non-zero"
        colMat = []
    ...
    def extract_image_chips(self, img, points, desired_size=256, padding=0):
        ...
        ----------
            img: (ndarray [1, 3, n, m]) bgr input image
            points: (ndarray [n x 10]) (x1, x2 ... x5, y1, y2 ..y5) landmarks
            desired_size: (int) size of output image
            padding: (int) amount of padding
        Returns:
        -------
            crop_imgs: (list) cropped and aligned faces
        """
