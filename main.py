...
# Initialize constants and networks
model_file = "models/opencv_detector/face_detector_uint8.pb"
config_file = "models/opencv_detector/face_detector.pbtxt"
for file in [model_file, config_file]:
    assert os.path.exists(file), "Could not find model file: {}".format(file)
...
def main(_):
    ...
    detections.load_model("models/facenet/facenet.pb")
    vcap = cv2.VideoCapture(0)
    embeds = np.load("data/embeddings.npy")
    user_data = pd.read_csv("data/user_data.csv")
    track_faces("kcf", vcap, 20, embeds, user_data)
